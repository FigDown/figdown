...
### 4.2 `table` — 配置/狀態表、記憶體映射（普查第 3，加權 9.8%）
...
`block-architecture`（普查第 1，加權 24.2%）**不需要型別區塊**——它
就是核心場景模型（§2）加上各種 `kind`；flowchart（8.3%，第 4）與
topology（5.0%）也收攏進同一模型。普查定案的算術：核心場景（37.5%）
+ bitfield（23.7%）+ table（9.8%）+ wave（7.2%）= **非模板圖出現
次數的 78%，約等於可分類圖形的 94%**（[census.zh-tw.md](census.zh-tw.md)）。
