...
- `fill` 是**區間帶**，以位置參數書寫：`fill 15%` = 0–15%
  ...
  範圍。可疊加、可作用於群組**或**單一節點。`dir=up|down|left|right` 決定量測軸與其 0% 起始
  邊（預設 `up`：0% 在底部——水位慣例；`right` 則呈現進度條式
  橫帶）。line 與 fill 是解耦的概念。
...
親戚。chart 家族在普查中屬少數（約 1%），故嚴格按需成長。
