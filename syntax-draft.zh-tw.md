...
橫跨群組框、位於其高度某百分比的導引線；`fill=below|above` 推導
著色區。這一個通用指令（無 id——沒有東西引用線）就涵蓋配額/
分割圖（欄位 = 群組內的透明節點）、水位線、hi/lo 門檻、以及未來
的圖表標記線。R28：它取代了整個原擬的 `partition` template
（4 個關鍵字 → 1 個）。
