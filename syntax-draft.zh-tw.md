...
> D1–D3）推導。此處沒有任何內容是定案。圖型優先順序將以正在進行的
> 圖型普查（774 份文件、1.2 萬張圖，R12）結果定案；初步數據已顯示
> 封包/位元欄位圖、方塊架構圖與表格三者合計約 72%，居於絕對主導。
>
...
pin l3 at=0.8,0.2               # 第 3 層：絕對位置（畫布比例）
...
- **尺寸適應**（R10）：有明確 `size` → 內容遷就框（字級可微降）；
  無明確尺寸 → 框最小幅度長大、不推動全局。
- OQ-S2：`at=` 的單位——畫布比例（0–1）vs 格子座標。目前草案傾向
  比例。
...
的 step——不用 `end` 關鍵字）。以下優先順序依初步普查，待完整結果
定案。

### 4.1 `bitfield` — 封包標頭/暫存器欄位（普查第 1，約 27%）
...
### 4.2 `table` — 配置/狀態表、記憶體映射（普查第 3，約 20%）
...
`block-architecture`（普查第 2，約 25%）**不需要型別區塊**——它就是
核心場景模型（§2）加上各種 `kind`。前兩大類分別由「核心場景」與
「bitfield」承接，這直接釘下 v0.1 的實作範圍：
**核心場景 + bitfield + table + wave ≈ 語料的 87%**。
