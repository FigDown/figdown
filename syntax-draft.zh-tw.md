...
- 引號字串內的跳脫：`\n` 換行、`\"` 字面引號、`\\` 字面反斜線。
  其他任何跳脫皆為行錯誤。選項值也可用引號：`label="on miss"`。
  （管線列另有 `\|` 與 `\^^`，§4.2。）
...
- OQ-S7：edge 標籤放在三個有意義的位置（尾/中/頭）。具體行內
  提案已上桌（R34）：`edge A [tail] <-[mid]-> [head] B`——運算子
  兩半繞著線中標籤拆開；`<-` 加入運算子集（R35）。主流調查與
  完整提案：[prior-art.zh-tw.md](prior-art.zh-tw.md) §1。待凍結；
  若採納，`label=`／`taillabel=`／`headlabel=` 附機械式遷移退役。
