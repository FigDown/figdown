...
//   reference/**.md       <- read/0.1/**.md          (GENRE-REFERENCE-ADDRESS)
//
// SKILL.md is still hand-maintained SOURCE; this script neither writes nor
// deletes it.
//
// WHY reference/ IS GENERATED. `read/0.1/` is the source of truth an ordinary
// reader of this repository is sent to — nothing to install, and a version
// directory so that `read/0.2/` can be added later while `read/0.1/` is never
// touched again. The bundle still needs its own copy, because `skill/figdown/`
...
// `skill/figdown/reference/`; edit `read/0.1/` and re-run this script.
'use strict';
const fs = require('fs');
...
// reference/: mirror read/0.1/ exactly. Files that no longer exist upstream are
