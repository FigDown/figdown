...
- `figdown-diff.js` — the semantic diff, and **not a gate**: it answers the one
  question a reviewer of a `.fd` revision has, *did the meaning change or only
  the drawing?*
  `node tools/figdown-diff.js A.fd B.fd` ·
  `node tools/figdown-diff.js --git <revA> <revB> <path>` ·
  `--json` · `--selftest`.
  It parses both sides with the engine, projects each through
  `conformance/normalize.js` — the same canonical model projection the
  conformance goldens use — and classifies every model field as MEANING or
  PRESENTATION against the table in `decisions/registry.md`.
  Output is a verdict line (`identical` · `comment-only` ·
  `presentation-only` · `semantic`) and two itemized sections; exit codes are
  0 · 0 · 3 · 4, with **exit 2 when either side fails to parse** — a diff over
  a document the engine rejected would report facts about a model no reader
  will ever see. Zero model calls, no clock, no network: same inputs, same
  bytes. The rule it is built to keep is ZERO FALSE SEMANTIC FACTS, so where it
  cannot pair two elements with certainty it reports a removal and an addition
  instead of guessing a rename or a label edit — including where several
  elements share one identity key, which are aligned as a MULTISET (unchanged
  occurrences first, then one leftover each side, then add/remove), because
  ordinal pairing after an insertion fabricates attribute changes. Three
  backstops: an unrecognised model key counts as MEANING and is named in a
  note; a safety net compares each model key and fires on any collection whose
  own routine reported nothing; and a 55-site audit asserts that every
  (element, presentation attribute) pair in the language is visible to the
  walk. `--selftest` runs 40 checks — 36 authored pairs, the audit, and 3 real
  revision pairs read with `git show` that skip rather than fail when the
  revisions are not there.
  Stage 1 of ADV-1: no canonical serialization, no semantic hash, no
  normativity — this tool reads and decides nothing about the language.
...
- `make-skill.js` — regenerates the self-contained agent-skill bundle
  ...
  engine source, and `reference/**.md` mirrored from `read/0.5/`, which
...
- `skill-coverage.js` — the gate that the **agent skill** teaches the whole
  ...
  `read/0.5/`, which is the source of truth (`GENRE-REFERENCE-ADDRESS`); the bundle's
...
3. **Transcribe** — follow [`read/0.5/transcribe.md`](../read/0.5/transcribe.md):
