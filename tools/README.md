...
- `fence-check.js` — verifies every ` ```figdown ` fence **and every FigDown
  ...
  (default roots: repo root `*.md` plus `spec/`, `design/`,
  ...
  **A self-test guards the migration-citation matcher.** The
  ...
  exception: `(MIGRATIONS 0.1)` is a CITATION FORMAT, and it is what covers
...
- `artifact-check.js` — the gate that compares an artifact against the source
  ...
  repository could see that class of defect, and it had shipped twice. See
  below.
...
- `make-skill.js` — regenerates the self-contained agent-skill bundle
  ...
  engine source, and `reference/**.md` mirrored from `read/0.1/`, which
...
- `capability-coverage.js` — the gate that the **example corpus** demonstrates
  ...
  closed-enum value including `flow`'s and `chart`'s; the comma forms
...
- `skill-coverage.js` — the gate that the **agent skill** teaches the whole
  ...
  `read/0.1/`, which is the source of truth (`GENRE-REFERENCE-ADDRESS`); the bundle's
  ...
  *A check that lists what it knows about cannot report the arrival of
  ...
  positions in `spec/`, `design/` and `conformance/` (`### `CLOSED-GRAMMAR` — …`,
...
- `migrate-figdown.js` — detection-based, idempotent spelling migration
  ...
  **Guard:** the tool hard-refuses `--write` over `conformance/cases/`,
  ...
  Retirement fixtures are *supposed* to contain retired spellings; a `--write` run "migrated" four of them into passing documents
...
project root). **Recursive** — 66 artifacts, against 28 for the top level of
those two directories alone. A `.fd` named on the command line is resolved to
its paired artifact. Engine lookup order: `$FIGDOWN_HTML`, co-located
`figdown.html`, `../editor/figdown.html`; the current version is read out of
...
| `no-engine-version` | built before 0.1, so the attribute does not exist. **No version is inferred** — the absence is itself the information (MIGRATIONS). Counted on its own line, neither warned nor failed |
| `skip` | `no-metadata` (an `.svg` with no `figdown-source` element) or `no-source` (metadata, but the paired `.fd` is absent). Both are **counted and listed**, never silently omitted |
...
opinion. `--strict` adds the engine-version mismatches, which are a WARN on
their own (`RENDERING-DETERMINISM` promises byte-identical output for the same source **and** the
same renderer version, so a lagging artifact is outside the promise even when
its source matches). `--verbose` prints every artifact rather than only the
flagged ones.
...
whole language and mark experimental constructs inline inside the fence; and
`design/`, an append-only research record. **The teaching-guide exclusion is a
...
`spec/`, `design/`, `skill/` (recursive). Collaborator and archival
...
3. **Transcribe** — follow [`read/0.1/transcribe.md`](../read/0.1/transcribe.md):
