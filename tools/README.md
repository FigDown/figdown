...
- `make-skill.js` — regenerates the self-contained agent-skill bundle
  ...
  engine source, and `reference/**.md` mirrored from `read/0.2/`, which
...
- `skill-coverage.js` — the gate that the **agent skill** teaches the whole
  ...
  `read/0.2/`, which is the source of truth (`GENRE-REFERENCE-ADDRESS`); the bundle's
...
genre (`block`, `topology`, `flowchart`, `statechart`) is counted separately
as `no-scene-in-scene-genre` and does fail. `gate:layout` runs with `--strict`.
...
3. **Transcribe** — follow [`read/0.2/transcribe.md`](../read/0.2/transcribe.md):
