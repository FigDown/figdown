...
with a mechanical migration.

**Resolved (2026-07-10)**: adopted and landed as migration 0.1-dev.9.
Bracket content rules per the user's follow-up ruling: balanced inner
brackets nest verbatim (`[flags[3:0]]` displays `flags[3:0]`);
unbalanced brackets / `\n` / literal quotes use the quoted form
`["..."]` with the standard string escapes (Mermaid's
quote-inside-shape precedent); empty `[]` is a line error.
`label=`/`taillabel=`/`headlabel=` retired; the migration was executed
mechanically across the gallery by a script implementing the 0.1-dev.9
rule — proof the rule is truly mechanical (R31). The user also
re-affirmed OQ-S4 stays out: conservative now; additions are cheap
later under R31 if real figures demand them.
...
exactly `-- -> <- <->` (precedent). Landed with the R34 rework
(migration 0.1-dev.9) so the operator registry changed once, not
twice.
...
### R38 — A daily-driver editor: draw.io-league UX, text-first (2026-07-10)

**Original**: I need a powerful editor whose feel is close to
draw.io/Visio — but not figure-first: the text must stay the convenient
primary view. Extend the current figdown.html into a genuinely
convenient editor that can load/save `.fd` files — at minimum
comfortable for my own daily use.

**Interpretation**: The editor is the human-side adoption vehicle
(R6), and axiom 7 already fixes its architecture: draw.io-league
direct manipulation where **every GUI action is a text edit**.
Implemented in the PoC: real file handles via the File System Access
API (open a `.fd`, Ctrl+S writes back to the same file; picker/download
fallback elsewhere), Save As / New / SVG export named after the file,
undo/redo of all edits (GUI actions snapshot individually, typing
coalesces), localStorage autosave with session restore, dirty marker +
discard confirmation, double-click empty canvas → new node + pin at
the click point, arrow-key nudge (Shift ×10) → rewrites pins, corner
resize handle → writes a `size` line, Del → removes the node with its
pins/edges, Esc → deselect, clickable errors that jump to the line.

### R39 — Installable agent skill: figures maintained hands-free (2026-07-10)

**Original**: Provide an agent skill — install it and an agent
automatically uses FigDown to maintain `.fd` files and embed the SVG
into `.md`; at minimum it should satisfy my own needs.

**Interpretation**: `skill/figdown/` is a self-contained bundle:
`SKILL.md` (trigger description, workflow, stability rules, R37
reading rules, the full cheat sheet), plus `build-svg.js` and the
engine (`figdown.html`) so the validate→render loop works with Node.js
alone — no clone, no network. Install =
`cp -r skill/figdown ~/.claude/skills/` (or a project's
`.claude/skills/`). The bundle is a build artifact regenerated by
`tools/make-skill.js` from the single engine source (never forked).
...
Status: **resolved (2026-07-10)** — user ruling on the analysis in
[prior-art.md](prior-art.md) §3: FigDown is **not** built on D2 as a
base language (no MD read-side ecosystem to inherit; D2's open grammar
is incompatible with the closed-grammar axiom; no layout-stability
promise; TALA proprietary). Borrowing D2's language design continues
(operator set, options). The remaining sub-question — a `figdown → d2`
exporter for interop — is re-filed as a v0.2 item.
...
- ~~**OQ1**: Build on D2 vs. new language~~ — resolved 2026-07-10: not
  a base language; a `figdown → d2` exporter re-filed as a v0.2 interop
  item (see the OQ1 section and [prior-art.md](prior-art.md) §3).
