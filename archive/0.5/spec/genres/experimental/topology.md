...
edges are physical or logical links rather than dataflow steps. Computer
networks are this genre's DOMINANT instance, not its definition: a PCIe
tree, a storage fabric, a cluster interconnect and a sensor mesh are the
same referent — devices, links, ports, bundles — and each of those domains
already calls its own such figure a "topology", with the qualifier varying
and the noun constant.
...
**When not to reach for it.** When the boxes are functional parts and the
lines are dataflow steps rather than devices and links, the figure is a
`block` — identity decides the genre, and it is read down the table until a
row describes the figure as a whole (`GENRE-SELECTION-PRECEDENCE`). It is also not the way to get a
paint order: a set of elements forming a logical layer of the subject is a
`class=` whose label says so (`PAINT-ORDER-CONSTRUCT`).
...
| `edge` | `edge <a> [tail] <op> [head] <b>` | T | NORMATIVE | `stroke` `style` `class` `note` (requires `figdown 0.3`) `id` (requires `figdown 0.5`) | op is written form; `[mid]` splits the operator; all three labels take the line's colour (`LABEL-COLOUR-SOURCE`); an edge is a LINK, physical or logical |
| `bundle` | `bundle <id> ["label"] <member>,<member>` | T | **EXPERIMENTAL** | `stroke` `style` | a LAG, an ECMP set, an EVPN Ethernet Segment — see the declaration below. Ring drawn dashed; a member is an endpoint pair `a--b` **or**, from `figdown 0.5`, a connector `id` (`CONNECTOR-IDENTITY-KEY`); member list is ONE whitespace-free comma-delimited token (the space form was RETIRED at 0.1); no `fill=` — a ring has no interior (§8.4) |
...
#### `bundle` — EXPERIMENTAL
    ...
    bundle <id> ["label"] <member>,<member>   [stroke=] [style=]
    member = <a>--<b> | <connector-id>          ; the id form: figdown 0.5
...
**A member names its link in one of two ways, and the second is what makes
the construct's own defining case expressible (`CONNECTOR-IDENTITY-KEY`).**

- an **endpoint pair**, `a--b`, which resolves when EXACTLY ONE connector
  runs between those two nodes;
- a **connector id**, `lag1a`, naming a connector declared with `id=`
  (core §10, `figdown 0.5`) in this section.

The two are told apart by lexis and not by a flag: `--` cannot occur inside
an id (`LINK-OPERATOR-IN-IDS`), so a member either contains it and is a pair or does not and is
an id. **A mixed list is legal** — every member resolves independently, and
forbidding the mixture would be a rule with no defect behind it. The pair
form is the one that reaches an **anonymous** connector, which has no id to
name; the id form is the one that reaches **parallel** connectors, which no
pair can address. That is why both stay: neither is a spelling variant of
the other, and each accepts input the other cannot.

**Why the id form was owed.** A LAG is *parallel links between one pair of
devices* — the definition's own words — and at N > 1 links the pair form
does not resolve. Until `figdown 0.5` this genre had a construct for a LAG
that could not express a LAG, and every authored bundle in the published
corpus routed around it. The network-topology model this genre's referents
come from legislates against the same failure: RFC 8345 §6.2 keys a link by
a mandatory identifier of its own and gives *"it is possible to have several
links between the same source and destination nodes"* as the reason it must.

```figdown
figdown 0.5 topology
node sw1 "Switch-1"
node sw2 "Switch-2"
edge sw1 -- sw2 id=lag1a
edge sw1 -- sw2 id=lag1b
edge sw1 -- sw2 id=lag1c
bundle lag1 "LAG-1 (802.1AX)" lag1a,lag1b,lag1c
```
...
**WHAT THE RING ENCLOSES IS PART OF WHAT THE BUNDLE SAYS (
`LASSO-ENCLOSURE-TRUTH`).** The ring is the drawn extent of the member links, so a device drawn
wholly inside it is drawn as one of them. That is a claim the source never
made, and it is refused rather than drawn: if a bundle's ring would enclose a
node that is not an endpoint of any member link, **the figure is not drawn**
and the diagnostic names the bundle's line, the bystander, and the two ways
out — move the coordinate that put it there, or change what the bundle
collects.

    Line N: the lasso for bundle "lag" would enclose non-member "ts" — a lasso
    is the drawn extent of the bundle's members, so this draws "ts" as one of
    them. Move the pin clear of the bundle's extent, or change what "lag"
    collects so the lasso is its members' own. The figure is not drawn rather
    than drawn wrongly.

Where no coordinate is the author's, the layout moves the bystander out
before the ring is drawn, at no cost to the author; where it cannot, the
message says *the layout pass could not separate them* and the figure is
still not written. This is the rule `group` has always had for its band
(§2.6: a band is the bounding box of its members, and a non-member inside it
is refused), asked of the other shape derived from a declared membership —
and it inherits that rule's principle whole: **whoever chose the position
bears the responsibility**. A **one-member** bundle draws no ring at all, claims no extent, and is never subject to this rule.

In the artifact the ring carries `data-lasso="<bundle id>"`, and every node
the ring was derived from carries the same id — so a reader can tell a member
from a bystander by identity, never by distance.
...
| `id` | a bare id (core §1) — the connector's HANDLE, on `edge` (`CONNECTOR-IDENTITY-KEY`). Unique within the section, in the same namespace as `node`/`group`/`external`/region ids, so a repeat is `duplicate id`. It is a handle and its SPELLING means nothing: `edge lag_member_1` does not say the link is a LAG member — `bundle` says that, or nothing does | NORMATIVE since `figdown 0.5` | absent — an anonymous connector is the ordinary case |
...
### The port is DRAWN, not only named (backlog 57b)

From 0.4 each crossing carries a **small square straddling the
device border**, half inside and half outside, stroked and filled in the
owning device's own colours, with the link attaching to it and the interface
name labelling it. The name is anchored to ITS square; where one border side
carries several ports the names serialize along that border **in square
order**, and a name that had to be written more than the standoff away from
its own square is joined to it by a hairline **leader**, so the association is
drawn rather than inferred from proximity. Two links that cross at the same
point share one square, which is what a shared fitting is.

**Prior art, and it is named rather than invented.** This is the port notation
of **OMG UML 2.5.1 §11.4** (composite structure): a Port is shown as a small
square on the boundary of the classifier that owns it, the connector attaches
to the square, and the port's name labels it. FigDown adapts the notation
without adopting UML's semantics — there is no Port metaclass here, no typed
interface, no required/provided distinction; the square is the drawing of a
fact this genre already carried, namely that an interface belongs to a device.
The claim about what UML says is registered in
[`spec/standards-claims.tsv`](../../standards-claims.tsv) (S210) with its
verification status, as every external-standard claim in this repository is.

Like the placement above, this is a DRAWING rule under `DOMAIN-CONVENTION-DIRECTIVES`: no syntax, no
model field and no option key. A `topology` figure whose `.fd` did not change
draws differently from 0.4, and its artifact is rebuilt.
...
namespace), dangling edge endpoint, ambiguous, unknown or duplicate `bundle`
member, and the single-valued-directive rules of §8 — all line errors.
(`in=` cycle is
...
**One `bundle` member-list error is new (`MEMBER-LIST-DUPLICATION`).** The same
edge named twice in one `bundle`'s member list is a line error —
`bundle t1 "T" a--b,a--b` used to parse silently into a two-member bundle
over one link (identity-model-proposal probe 5, a live defect independent
of edge identity). The test is on the edge each member RESOLVES to, not on
the spelling, so a reversed spelling of the same undirected pair
(`a--b,b--a`) is caught too:

```
Line N: duplicate bundle member "a--b" — each member is stated once
```

**This is a within-one-list duplicate, not a repeated-construct duplicate,
and core §8.1's table does not carry a row for it.** §8.1 answers "is
writing THIS CONSTRUCT twice an error", across lines or sections — the
class `edge` and `rank` (accepted) and `title`/`pin`/a duplicate id
(rejected) belong to. A `bundle` member repeated inside ONE list argument
of ONE directive occurrence is the other kind of repeat, the one core §8
already carries outside that table: "a repeated option key on ONE line is
a line error" (the option-lexer bullet) is the precedent this follows, not
a construct-repeat rule extended to cover it.

**Five `bundle` member errors are new or reworded (`CONNECTOR-IDENTITY-KEY`),
because a member may now be a connector id.** Each names what the author
wrote, in the form they wrote it:

```
Line N: "a--b" matches 2 edges — the pair form addresses a connector only where ONE runs between the two nodes. Give the connector you mean an id= and write that id as the member (figdown 0.5)
Line N: bundle member "ghost" is not a declared connector in this section (ids are unique per section, and a bare member names a connector by its id=)
Line N: bundle member "a" is a node, not a connector — a bundle collects LINKS, so write the pair A--B or give the connector you mean an id= and name that
Line N: bad member "a.b" (expected A--B or a connector id)
```

The first REPLACES the pre-0.5 message, which said the case was *"out of
scope for v0.1"*. That message was wrong in three ways in an engine past
`0.1`: it cited a version the document was not written against, it stated a
scope exclusion where the truth is *say which one you mean*, and it named no
remedy. There is a remedy now, and the message is where the author meets it.

The third names the KIND the id actually resolves to — a node, a group, an
`external` endpoint, a region, a class or a bundle — because the kind is the
whole of the author's next move.

**Two cases deliberately get NO message of their own.** An id member and a
pair member that resolve to the SAME connector are caught by `MEMBER-LIST-DUPLICATION`'s
duplicate rule unchanged, because that rule was bound to the resolved edge
rather than to the spelling for exactly this reason. And an id declared in a
NEIGHBOURING SECTION gets the *"not a declared connector in this section"*
message above: ids are unique per section (core §1) and a cross-section
reference is a line error by design, so there is nothing else true to say —
a *"declared in another section"* message would imply a resolution model the
language does not have.

**Below `figdown 0.5` a bare member is still `bad member "xx" (expected
A--B)`.** Naming the id form to a document whose header cannot write it
would send the author to a key they do not have.
...
- **The genre NAME stays the bare word `topology` — ruled 2026-08-20, and the
  ruling joins the same promotion gate as the item above.** The alternative
  considered was `network-topology`, reserving the bare word for a future
  generic genre. Rejected on the record: the referent stated at the top of
  this document IS the generic one — every domain that draws such a figure
  (networks, PCIe, storage fabrics, clusters, meshes) shares the same
  semantic model and names it with the same bare noun under a varying
  qualifier, so a qualified name would shrink the referent artificially and
  invite per-domain prefix fragmentation with identical vocabularies; and
  `network` itself is already reserved for a different job (the layering
  relation, first bullet family above), so `network-topology` would sit one
  word away from a construct that means something else. The price paid,
  written next to the clause: a future topology genre whose model is NOT
  devices-and-links has no word — and no such figure has appeared in any
  measured population. *Reopens if:* a corpus-measured need arrives for a
  topology figure the devices-and-links model cannot serve.
