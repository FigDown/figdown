...
**When not to reach for it.** When the machine's story is really a
time-ordered exchange between two participants — messages sent back and
forth — the figure is a `sequence`, and the ladder gives each message its own
row instead of crowding them into one span. Many edges between the same pair
of states do **not** meet that test: where they are distinct transitions on
distinct triggers the figure stays here and the crowding is a layout problem,
which is how `bfd-session.fd` was decided
(`decisions/registry.md`, the genre-choice heuristic).
...
| `transition` | `transition <a> [tail] <op> [head] <b>` | **S** | **EXPERIMENTAL** | `stroke` `style` `class` `note` (requires `figdown 0.3`) `id` (requires `figdown 0.5`) | op is written form; `[mid]` splits the operator and carries the inscription; all three labels take the line's colour (`LABEL-COLOUR-SOURCE`). OMG UML 2.5.1 §14 |
...
**One option key joins this genre's connector at `figdown 0.5` (
`CONNECTOR-IDENTITY-KEY`): `id=`, the transition's optional HANDLE.** A bare id (core §1), unique
within the section and in the shared id namespace, so a repeat is `duplicate
id`. It matters more here than anywhere else in the language and means less
than an author will expect: `statechart` holds the largest population of
parallel connectors in the published corpus — two transitions between one pair
of states are two different TRIGGERS — so this is where naming one is most
often useful, and it is also where over-reading a name would cost the most.
The id's SPELLING asserts nothing (core §12.7). The trigger is the whole
content of the arc and it lives in the `[mid]` inscription; a transition
named `timeout` states no trigger, and reading rule 11's ban on inferring
meaning from presentation holds for the handle exactly as it holds for the
paint.
