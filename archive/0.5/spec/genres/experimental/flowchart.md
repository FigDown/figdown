...
**When not to reach for it.** When the boxes are modes the system is *in*
rather than steps that are *performed*, the figure is a `statechart`, and
nothing in the drawing will say so — the header is the only place either can
be declared. This genre also declares no `group` and takes no `in=`
(`SCENE-KEYWORD-MEMBERSHIP`, `MEMBERSHIP-KEY-ACCEPTANCE`), so a procedure whose content is nested regions has a positive
reason to be a `block`: capacity, not portability (`GENRE-SELECTION-PRECEDENCE`).
...
| `flowline` | `flowline <a> [tail] <op> [head] <b>` | **F** | **EXPERIMENTAL** | `stroke` `style` `class` `note` (requires `figdown 0.3`) `id` (requires `figdown 0.5`) | **`edge` under this genre is a line error (`GENRE-CONNECTOR-SPELLING`)**; op is written form; `[mid]` splits the operator; all three labels take the line's colour (`LABEL-COLOUR-SOURCE`) |
...
| `id` | a bare id (core §1) — the connector's HANDLE, on `flowline` (`CONNECTOR-IDENTITY-KEY`). Unique within the section, in the shared id namespace, so a repeat is `duplicate id`. Its SPELLING means nothing: two flowlines between one pair of stages are two CONDITIONS, and naming them `yes`/`no` states neither condition — the `[mid]` label is what states it | NORMATIVE since `figdown 0.5` | absent — an anonymous flowline is the ordinary case |
