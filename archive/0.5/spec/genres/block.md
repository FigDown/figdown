...
| `edge` | `edge <a> [tail] <op> [head] <b>` | B | NORMATIVE | `stroke` `style` `class` `note` (requires `figdown 0.3`) `id` (requires `figdown 0.5`) | op is written form; `[mid]` splits the operator; all three labels take the line's colour (`LABEL-COLOUR-SOURCE`) |
...
| `id` | a bare id (core §1) — the connector's HANDLE, on `edge` (`CONNECTOR-IDENTITY-KEY`). Unique within the section and in the shared id namespace, so a repeat is `duplicate id`. Its SPELLING carries no meaning; what an id is FOR is being named by something else, and in a `block` figure nothing yet names one — the key is legal here because the connector is the same connector in every scene genre, not because this genre has a consumer for it | NORMATIVE since `figdown 0.5` | absent |
...
- Explicit element attributes override class defaults (rigidity, `LAYOUT-STABILITY`) —
  that boundary is unchanged. **Between two carried classes there is no
  cascade: each paint channel (`fill`, `stroke`, `style`) must come from
  exactly one of them, and a second carried class setting a channel the
  first already set is a line error** (`CLASS-CHANNEL-COLLISION`) — `classes
  "tagged" and "untagged" both set stroke= on this element — one channel,
  one class: move one class's paint to a different channel (fill=/style=),
  or carry only one of them`. This is core §8's own rule ("a repeated
  option key on ONE line is a line error, never last-wins") one level up:
  a class's look is how the legend tells it apart, and two classes on one
  channel means the drawing shows one and the model claims two.
  `class=hot,deprecated` above stays legal because it PARTITIONS: `hot`
  claims `fill`/`stroke`, `deprecated` claims `style` — one source per
  channel.
