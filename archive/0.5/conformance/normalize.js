...
//                                             even for display)
//   edge  : id?, a, op, b, tail, mid, head, plane, stroke, style,
//           class, line   (0.1: no `fill` — an edge has no interior,
//           so fill= and stroke= named one channel; core §5)
//           (CONNECTOR-IDENTITY-KEY, 0.5: `id` is the connector's optional handle from
//            `id=`, present ONLY when written and first when present. An
//            anonymous connector is the language's default and the absent key
//            is its whole statement — §12.3's absence rule, not a null)
//   rank  : ids, line
...
//                                            (0.1: no `fill`, core §5)
//                                            (members as "a--b" strings, and
//                                             from CONNECTOR-IDENTITY-KEY also as
//                                             a bare connector id — ONE array
//                                             of ONE type, the two forms told
//                                             apart by `--`, which LINK-OPERATOR-IN-IDS bans
//                                             inside an id)
...
//   message  : id?, a, op, b, label, tail, head, in, stroke, style, class,
//              note, description, line
//              (CONNECTOR-IDENTITY-KEY, 0.5: `id` is the same optional handle the other
//               three connectors carry. It orders nothing and equates nothing:
//               the total order is still `line`)
...
function normalize(doc) {
  ...
  if ((doc.boundaries || []).length)
    model.externals = doc.boundaries.map(b => o([
      ...
      ['line', b.line]]));
  // CONNECTOR-IDENTITY-KEY: `id` — the connector's optional handle, from `id=`. It
  // is FIRST, matching every other id-bearing element in §12.2, and it is
  // PRESENT ONLY WHEN WRITTEN, so no golden model of an anonymous edge moves a
  // byte in either position. The key carries no meaning beyond being a handle:
  // §12.7's reading contract forbids reading a category, an ordinal or a
  // relationship out of an id's spelling.
  model.edges = (doc.edges || []).map(e => o([
    ['id', e.id], ['a', e.a], ['op', e.op], ['b', e.b],
  ...
  // CONNECTOR-IDENTITY-KEY: `members` is projected from the AUTHORED member list,
  // which is where the two member forms stay distinguishable — `"a--b"` for a
  // pair, `"e2"` for a connector id. The SCHEMA does not change: it was already
  // "array of string", and the string domain widens by one shape that `--`
  // tells apart from the other (LINK-OPERATOR-IN-IDS). No golden model gains a key.
  model.bundles = (doc.trunks || []).map(t => o([
    ['id', t.id], ['label', t.label],
    ['members', (t.members || t.pairs.map(p => ({ a: p[0], b: p[1] })))
      .map(m => (m.id !== undefined ? m.id : m.a + '--' + m.b))],
    ['stroke', t.stroke],
  ...
  const seq = [
    ...
    // CONNECTOR-IDENTITY-KEY: `id` is first here for the same reason it is first on
    // an edge, and it means the same thing — a handle, present only when
    // written. It is NOT an ordering key and NOT an identity claim between two
    // occurrences: the model's total order is still `line`, and the diff still
    // aligns messages as a multiset.
    ['messages', (doc.messages || []).map(m => o([
      ['id', m.id], ['a', m.a], ['op', m.op], ['b', m.b],
