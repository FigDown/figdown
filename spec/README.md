...
| [`ERRATA.md`](ERRATA.md) | the register of corrections to **frozen** release trees (`read/<X.Y>/`, `archive/`), whose bytes §13.5 and §13.7.3 hold immutable: where one of them was already wrong on the day it shipped, the bytes still stand and the correction is recorded here instead. An erratum is not a version change — the engine's behaviour did not move, so there is no `migrations.md` entry, no engine increment and no header bump — and because a frozen document cannot point forward to its own correction, this register is the only route by which one reaches a reader of that tree |
...
| [`standards-claims.tsv`](standards-claims.tsv) | the register of every claim this repository makes **about an external standard** — one row per assertion, not per sentence, carrying the clause it cites, the evidence in the form its claim kind demands, the sites that assert it, and a status that is never null. It exists because nobody held the list: three known false claims became thirteen once someone enumerated them, with twenty-five gates running and not one of them counting an external-standard claim. This file is that denominator, and `gate:standards` reads it |
