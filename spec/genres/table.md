...
**When not to reach for it.** When the knowledge is which part connects to
which rather than what sits in the cells, `block` carries it; when the grid is
the bit layout of a word on the wire, `bitfield` does. In the other direction
this genre is the destination rather than the fallback: a trend, bar or line
chart is deliberately expressed here, because the chart's knowledge is its
values and a table states them (`CHART-SCOPE-BOUNDARY`).
