...
Change:  新增外部 I/O 端點（spec §2.8）：`boundary <id> ["label"]`
         宣告「外界」——edge 可在任一端點像節點一樣引用它
         （bundle 成員亦可正常引用這類 edge），可 pin 排版，
         **永不**畫成形狀：edge 就此開放收尾、保留原本的箭頭，
         選填的標籤以小號淡色文字畫在開放端稍外。id 與
         node/group 共用命名空間（重複與跨類衝突皆為行錯誤）；
         `boundary` 不接受任何選項；多餘位置參數是行錯誤。真正
         未知的 edge 端點 id 仍是懸空端點錯誤。
         註冊表：頂層關鍵字 19 → 20。
Rule:    **純新增**——零改寫。未宣告 `boundary` 的文件，解析與
         渲染與先前完全相同。（先前以假節點模擬外部端點的作者，
         可把 `node ext "PKT"` 與其 edge 改為
         `boundary ext "PKT"`——選擇性、**非機械**：只有作者
         知道那個節點原意是參與者還是外界。）
Example: （無）→ `boundary wire "to wire"` ＋ `edge mac -> wire`。
