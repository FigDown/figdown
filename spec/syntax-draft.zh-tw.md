...
（`block`→right、`flowchart`→down）、預設單位——各自對準該圖型
的普查統計。template **不得**改變標準指令的核心語意：`node` 永遠
是節點、`edge` 永遠是關係——也永不拿走表達力：顯式語法（運算子、
選項）永不被 template 預設或限制。有向、帶色的 edge 在
`topology` 下與 `block` 下完全相同（實戰回饋 F1——某下游轉寫
因本段早期草稿的誤導而丟失方向語意）。新 template 必須同時具備
語料佐證**與**語意不可表達性（R28）。

**版本相容性。** 標頭攜帶線上文法版本與 template。未知主版本
**必須**拒絕。次版本的寬容規則依一致性模式分層（§10）：檢視器層
（寬鬆模式）實作**應**以寬鬆模式解析未知次版本；嚴格的創作層實作
**得**逕行拒絕未知次版本（參考引擎即如此）。未知 template 在
嚴格模式下**必須**拒絕。
...
- 顏色用 CSS hex（`#rgb` 或 `#rrggbb`，如 `#0d9488`）或 CSS 顏色名
  （147 個 CSS/SVG 色名關鍵字，小寫，另加 `transparent`）；其他任何
  值皆為行錯誤。
- `#` 只有在行首或前面是空白時才開始註解
...
`in=` 是**節點**選項；`group` 不接受它——v0.1 的嵌套僅一層
（節點入群組），因此 `group … in=…` 是行錯誤。
...
負責，且它們像 `pin`/`size` 一樣收在尾部 layout 區——永不寫在
`edge` 行內。
...
pin l3 at=420,80                # 第 3 層：位置（px，相對於元素的
                                #          定位上下文）
size l3 w=120 h=60              # 第 3 層：明確尺寸（僅 px）
routing orthogonal              # 第 3 層：直線 edge 改畫直角折線
route c -> a via=340,20;10,20   # 第 3 層：對單一 edge 宣告剛性途經點
```
...
- **尺寸適應**（R10）：有明確 `size` → 內容遷就框（字級可微降）；
  無明確尺寸 → 框最小幅度長大、不推動全局。v0.1 的尺寸**僅有
  px**——百分比尺寸保留給未來版本。
- **兩層釘點（D6）**：被釘的**群組**以畫布 px 錨定其局部原點；
  被釘的**成員**採群組局部座標（相對該原點）。因此移動群組是
  一行修改，群組內的編輯永不干擾其他群組。未分組的釘點為畫布 px。
- **Edge 走線（0.1-dev.13）**——展現層、第 3 層、不帶語意。
  `routing orthogonal|straight` 是文件級指令（預設 `straight`——
  即原本的直連線）。`orthogonal` 之下，普通直線場景 edge 改畫成
  確定性的曼哈頓直角折線——`flow right|left` 時先水平後垂直、
  `down|up` 時先垂直後水平——沿用與直線相同的邊框錨定規則；
  back-edge 通道本來就是直角走線，避障繞行維持原形。
  `route <a> <op> <b> via=x,y;x,y;… [routing=orthogonal|straight]`
  對**一條**既存 edge 宣告**剛性途經點**：引用必須與 edge
  **原文一致**（`a`、運算子、`b`——端點順序與運算子都算數，與
  `bundle` 成員不同）、必須解析到唯一一條 edge（引用平行邊為
  歧義＝行錯誤），且每條 edge 至多一筆 `route`（重複＝行錯誤）。
  途經點為畫布 px，與未分組 `pin` 同一座標空間；edge 畫成折線
  來源→via1→…→viaN→目標（走線為 orthogonal 時相鄰點之間以
  直角肘接——route 行上的 `routing=` 覆蓋文件級指令）。mid 標籤
  落在最長線段上（近垂直時貼在旁邊）；箭頭保留。與 `pin`/`size`
  相同，`routing`/`route` 是**不帶意義**的渲染參數，收在尾部
  layout 區——永不寫在 `edge` 行內（§2.3）。
- **語意完備不變量（R25）**：把文件中所有 `pin`、`size`、
  `routing`、`route` 行刪除後，必須仍可解析、仍可在自動排版下
  渲染、且表達一字不差的結構與關係。編輯器慣例上把物化的佈局
  收進尾部的 `# layout` 區，讓結構先被讀到。
- **OQ-S2 已定案：`at=` 為相對於元素定位上下文的 px**——未分組
  的節點與群組相對於畫布；群組成員相對於該群組的局部座標系
  （D6）。畫布比例在實測後否決（畫布一長大，所有比例釘點跟著
  漂移）。edge 永遠由節點邊框衍生——自動適應、不可釘。
- **雙區閱讀契約（R43，規範性）**：文件分語意區（上）與渲染區
  ——自第一個以 `# --- layout` 開頭的註解行起到檔尾。渲染區
  **只**得放渲染宣告（`pin` `size` `route` `routing`）、**不得**
  攜帶語意；語意區**不應**放只有渲染才需要的資訊。因此讀取端
  agent **可以**在 layout 標記處停止閱讀而零語意損失。
...
- 寬度以位元計；renderer 計算位元索引並繪製刻度尺。寬於本列剩餘
  空間的欄位**自動跨到後續列**（例如 128-bit 欄位跨四個 32-bit
  列）——它仍是「一個」欄位；作者**不得**手動拆成逐列片段，那會
  誤述欄位列表（R37）。
...
**語意模型（規範性——讀取規則，R37）。** bitfield 的意義恰恰就是
其有序欄位列表：第 *k* 個欄位占據 header 的
`[前 k-1 個欄位寬度總和, +w_k)` 位元區間，按宣告順序排列。
**任何地方都沒有隱含填充**——真實的 padding/reserved 區域**必須**
宣告為顯式欄位（防捏造規則）。`wrap` **純屬展現**：只是開始新的
繪圖列，永不增加、跳過或重排位元。`numbering=` 只改每列尺標的
標籤，永不改變擺放。標記 `optional` 的欄位是條件存在——其後欄位
的絕對 offset 只在它存在時成立。`*` 欄位之後的 offset 相對於其
變長終點。讀取端 agent **必須**由此模型推導 layout、**不得**由
繪圖幾何推斷位元（`wrap` 之後的空白格不是位元）。
...
**語意模型（規範性——讀取規則，R37）。** 表格的意義是其邏輯
網格：表頭層（分隔列以上的列，`h1..hN` 由上往下）、資料列
（1 起算），`||`／`^^` 產生以左上格為錨點的合併邏輯格。`cell`
標註與 `highlight` 是附著於網格定址的註記（「有註記」這件事是
語意；其顏色是展現）。`colw` 與對齊純屬展現，永不改變網格。
...
**語意模型（規範性——讀取規則，R37）。** wave 的意義逐訊號定義：
第 *t* 個 tick 的值 = 軌道第 *t* 個字元（`.` = 延續前值）。tick
在區塊內所有訊號間連續且對齊；`gap <tick>` 是純展現的斷點標記，
永不移除或重編 tick。`labels=` 依出現順序為 `=` 資料格命名。
...
機制，不設兩種）。edge 走線專屬於 layout 區的 `routing`/`route`
指令（§3）——永不寫在 `edge` 行。其餘（字型、間距、箭頭）屬於
renderer/主題，不屬於語言。

規範性邊界（「展現可忽略」不變量，R25 的延伸）：移除所有純展現
屬性（`color`、`stroke`、`text`、`style`、`gap`、`z`）與佈局指令
（`pin`、`size`、`routing`、`route`）**不得**改變文件的語意結構；
語意消費者**可以**忽略它們。因此**顏色與樣式不得是語意的唯一載體**——若顏色/虛線
...
- **渲染選項（renderer 層，非語言）。** renderer **可以**接受展現
  選項——v0.1 定義一個：`with-title`（在 SVG 內繪出標題）。
  **預設不繪**：嵌入的圖幾乎總在宿主文件的 caption 之下（R13——
  預設跟隨最大宗；主流工具亦不預設繪標題）。無論繪否，標題
  **文字**都是來源中的語意。任何非預設選項**必須**記錄於
  artifact metadata（`data-render-options="…"`），使產物保持為
  （來源、記錄選項）的純函數、第三方重建位元級一致。選項永不
  出現在來源文法中——註冊表不受影響，且選項詞彙保持最小
  （旋鈕跟語法一樣得證明自己的必要性）。
...
AGENT-GUIDE 速查表）。任何使頂層關鍵字超過約 20 個的新增，
必須通過 R28 關卡。
