...
- **渲染選項（renderer 層，非語言）。** renderer **可以**接受展現
  選項——v0.1 定義一個：`no-title`（嵌入文件自帶 caption 時不繪
  標題；標題**文字**仍是來源中的語意）。任何被接受的選項**必須**
  記錄於 artifact metadata（`data-render-options="…"`），使產物
  保持為（來源、記錄選項）的純函數、第三方重建位元級一致。
  選項永不出現在來源文法中——註冊表不受影響。
